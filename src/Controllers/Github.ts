...
/* resk start "GithubAuthentication" */
class GithubAuth {
  async get_token(req: Request, res: Response) {
    ...
    const url = `https://github.com/login/oauth/authorize?client_id=${
      process.env.GITHUBAPP_CLIENT_ID
    }&redirect_uri=${encodeURIComponent(
      redirect_uri,
    )}&scope=repo%20user%20delete_repo&state=${state}&allow_signup=false`;
