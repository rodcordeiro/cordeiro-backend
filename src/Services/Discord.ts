...
class DiscordService {
  async send_message(
    channel: string,
    message: string,
    options: iDefaultMessage = {},
  ): Promise<any> {
    return new Promise(async (resolve, reject) => {
      const wservices = new WebhookServices();
      let hook: any = await wservices
        .get_webhook_by_name(channel)
      ...
      await discordApi
        .post(hook, {
          ...
          avatar_url: options.avatar_url
            ? options.avatar_url
            : 'https://rodcordeiro.github.io/shares/img/rodrobo.jpg',
        })
        .then((response) => {
          resolve(response);
        })
...
export { iDiscordMessage, DiscordService };
